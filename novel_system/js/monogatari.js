...
(function (global) {
  ...
  class MonogatariEngine {
    ...
    parseCommandString(cmdStr) {
      ...
      // Command: show banner <title> | <subtitle> [| <duration_ms>]
      if (trimmed.startsWith('show banner ')) {
        const parts = trimmed.replace('show banner ', '').split('|');
        const mainTitle = parts[0] ? parts[0].trim() : 'CHAPTER';
        const subTitle = parts[1] ? parts[1].trim() : '';
        const duration = parts[2] ? parseInt(parts[2].trim(), 10) : 2000;
        this.renderChapterBanner(mainTitle, subTitle, duration);
        return;
      }

      // Command: show award <title> | <subtitle> [| <icon>] [| <duration_ms>]
      if (trimmed.startsWith('show award ')) {
        const parts = trimmed.replace('show award ', '').split('|');
        const mainTitle = parts[0] ? parts[0].trim() : '職位晉升通知';
        const subTitle = parts[1] ? parts[1].trim() : '';
        const icon = parts[2] ? parts[2].trim() : '👔';
        const duration = parts[3] ? parseInt(parts[3].trim(), 10) : 2500;
        this.renderAwardOverlay(mainTitle, subTitle, icon, duration);
        return;
      }
    ...
    renderInputPrompt(inputDef) {
      ...
      const handleSubmit = () => {
        ...
        if (typeof inputDef.Save === 'function') {
          inputDef.Save(finalVal);
        }
        if (inputDef.Storage) {
      ...
      const handleSkip = () => {
        const defaultVal = inputDef.Default || (inputDef.Storage ? this._storage[inputDef.Storage] : '') || '陸天成';
        if (typeof inputDef.Save === 'function') {
          inputDef.Save(defaultVal);
        }
        if (inputDef.Storage) {
    ...
    // Text Formatting Helper (Dynamic Storage Variable Placeholders & Character Name References)
    formatText(text) {
      if (!text) return '';
      return text.replace(/\{\{(?:storage\.)?([a-zA-Z0-9_\.]+)\}\}/g, (match, key) => {
        ...
        // Support {{characterId.name}} syntax (e.g. {{linwei.name}}, {{gulingfrost.name}})
        if (key.endsWith('.name')) {
          const charId = key.replace('.name', '');
          if (this._characters[charId] && this._characters[charId].name) {
            return this._characters[charId].name;
          }
        }
    ...
    renderDialogueLine(line) {
      ...
      // Check if line starts with character ID (support single ID or multi-ID separated by + or ,)
      const spaceIdx = line.indexOf(' ');
      if (spaceIdx > 0) {
        const potentialIdStr = line.substring(0, spaceIdx);
        const ids = potentialIdStr.split(/[\+,]/).map(s => s.trim());
        const validChars = ids.map(id => this._characters[id]).filter(Boolean);

        if (validChars.length > 0) {
          speakerName = validChars.map(c => c.name).join(' & ');
          if (validChars.length === 1) {
            speakerColor = validChars[0].color || '#ff4081';
          } else {
            const colors = validChars.map(c => c.color || '#ff4081');
            speakerColor = `linear-gradient(135deg, ${colors.join(', ')})`;
          }
    ...
    renderChoice(choiceObj) {
      ...
      Object.keys(choiceObj).forEach((optKey) => {
        ...
        btn.addEventListener('click', () => {
          this.choiceContainer.classList.remove('active');
          let actionResult = opt.Do;
          if (typeof opt.Do === 'function') {
            actionResult = opt.Do(this._storage);
          }
          if (typeof actionResult === 'string' && actionResult.startsWith('jump ')) {
            const targetLabel = actionResult.replace('jump ', '').trim();
            this.jump(targetLabel);
          } else {
            this.next();
          }
    ...
    // Render Cinematic Chapter / Level Transition Banner Overlay
    renderChapterBanner(title, subtitle, durationMs = 2000) {
      let banner = document.querySelector('#chapter-banner-overlay');
      if (banner) banner.remove();

      banner = document.createElement('div');
      banner.id = 'chapter-banner-overlay';
      banner.style.cssText = `
        position: absolute;
        inset: 0;
        z-index: 99;
        background: linear-gradient(135deg, rgba(15, 12, 28, 0.95), rgba(28, 20, 50, 0.96));
      ...
      `;
      banner.innerHTML = `
        <div style="font-size: 13px; letter-spacing: 5px; color: #c084fc; text-transform: uppercase; margin-bottom: 12px; font-weight: 700; text-shadow: 0 0 10px rgba(192, 132, 252, 0.5);">STAGE TRANSITION</div>
        <div id="banner-title" style="font-size: 32px; font-weight: 800; color: #ffffff; text-shadow: 0 4px 24px rgba(168, 85, 247, 0.7); margin-bottom: 12px; letter-spacing: 2px; text-align: center; padding: 0 20px;"></div>
        <div id="banner-sub" style="font-size: 18px; color: #e2e8f0; font-weight: 500; text-shadow: 0 2px 8px rgba(0,0,0,0.5); text-align: center; padding: 0 20px;"></div>
        <div style="margin-top: 36px; font-size: 12px; color: #94a3b8; letter-spacing: 2px; animation: pulse 1.5s infinite;">- 點擊螢幕繼續 -</div>
      `;

      if (this.container) this.container.appendChild(banner);

      const tEl = banner.querySelector('#banner-title');
      const sEl = banner.querySelector('#banner-sub');
      if (tEl) tEl.textContent = this.formatText(title);
      if (sEl) sEl.textContent = this.formatText(subtitle);

      // Play audio chime
      if (global.app && global.app.playChimeSound) {
        global.app.playChimeSound();
      }
      ...
      const timeoutDuration = (typeof durationMs === 'number' && !isNaN(durationMs)) ? durationMs : 2000;
      ...
      const finish = () => {
        ...
        banner.removeEventListener('click', finish);
      ...
      banner.addEventListener('click', finish);
    }

    // Render Settlement Award / Achievement Popup Overlay
    renderAwardOverlay(title, subtitle, icon = '👔', durationMs = 2500) {
      let overlay = document.querySelector('#award-popup-overlay');
      if (overlay) overlay.remove();

      overlay = document.createElement('div');
      overlay.id = 'award-popup-overlay';
      ...
      overlay.innerHTML = `
        <div style="
          background: linear-gradient(135deg, rgba(30, 25, 55, 0.95), rgba(45, 30, 80, 0.95));
          border: 1px solid rgba(234, 179, 8, 0.5);
          box-shadow: 0 0 35px rgba(234, 179, 8, 0.35), inset 0 0 15px rgba(234, 179, 8, 0.15);
          border-radius: 16px;
          padding: 28px 36px;
          text-align: center;
          max-width: 420px;
        ...
        ">
          <div style="font-size: 44px; margin-bottom: 12px; filter: drop-shadow(0 0 10px rgba(234, 179, 8, 0.6));">${this.formatText(icon)}</div>
          <div style="font-size: 12px; font-weight: 700; color: #eab308; letter-spacing: 3px; text-transform: uppercase; margin-bottom: 8px;">STAGE CLEAR AWARD</div>
          <div id="award-title" style="font-size: 24px; font-weight: 800; color: #ffffff; text-shadow: 0 2px 10px rgba(0,0,0,0.5); margin-bottom: 8px;"></div>
          <div id="award-sub" style="font-size: 16px; color: #cbd5e1; font-weight: 500;"></div>
          <div style="margin-top: 20px; font-size: 11px; color: #94a3b8; letter-spacing: 1px;">- 點擊螢幕繼續 -</div>
        </div>
      `;

      if (this.container) this.container.appendChild(overlay);

      const tEl = overlay.querySelector('#award-title');
      const sEl = overlay.querySelector('#award-sub');
      if (tEl) tEl.textContent = this.formatText(title);
      if (sEl) sEl.textContent = this.formatText(subtitle);
      ...
      let autoTimer = setTimeout(finish, durationMs);
